...
# Inline MTA mode (adds vigilyx-mta; mirror mode and MTA mode are separate traffic paths)
...
If you deploy inline MTA instead of passive mirror capture, replace `--profile mirror` with `--profile mta` in the commands above.
...
```bash
# Stop database writers before restore
cd /home/vigilyx/vigilyx/deploy/docker
docker compose stop vigilyx
# If you use inline MTA mode, also stop the MTA writer:
# docker compose stop mta
