...
在 mirror 模式下，sniffer 负责抓流量并重组会话，再通过 Redis Streams 进入 engine。engine 把结果写入 PostgreSQL，最后由 API 与前端展示。

## Inline 路径

在 MTA 模式下，SMTP 代理接收邮件、解析消息、执行 Inline 检测，并决定是转发、隔离还是拒收。
