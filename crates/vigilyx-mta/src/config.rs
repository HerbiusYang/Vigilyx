...
impl MtaConfig {
    ...
    pub async fn override_from_db(&mut self, db_url: &str) -> anyhow::Result<()> {
        ...
        let row: Option<(String,)> = sqlx::query_as(
            "SELECT value FROM config WHERE key = 'deployment_mode'"
        ...
        {
                if let Some(h) = val.get("mta_downstream_host").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
                ...
                if let Some(d) = val.get("mta_local_domains").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
                    let mut domains: Vec<String> = d.split(',')
                ...
                if let Some(a) = val.get("mta_dlp_action").and_then(|v| v.as_str()) {
                    match a.to_lowercase().as_str() {
                        ...
                        "allow" | "allow_and_alert" => self.dlp.action = crate::dlp::DlpAction::AllowAndAlert,
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let downstream_host = std::env::var("MTA_DOWNSTREAM_HOST")
        ...
        Ok(Self {
            ...
            outbound: std::env::var("MTA_OUTBOUND_HOST").ok().map(|host| {
                ...
                let starttls = std::env::var("MTA_OUTBOUND_STARTTLS")
                    ...
                    .unwrap_or(true);
                DownstreamConfig { host, port, starttls, timeout_secs: 30 }
            }),
            local_domains,
