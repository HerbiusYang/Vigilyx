...
pub struct TcpPendingSegmentsDiag {
    pub waiting_for_seq: Option<u32>,
    pub pending_segments: usize,
    pub pending_bytes: usize,
    pub first_pending_seq: Option<u32>,
    pub first_pending_len: usize,
    pub first_gap_bytes: usize,
    pub last_pending_seq: Option<u32>,
    pub last_pending_end_seq: Option<u32>,
    pub last_pending_len: usize,
}

impl fmt::Display for TcpPendingSegmentsDiag {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if self.pending_segments == 0 {
            return match self.waiting_for_seq {
                Some(waiting_for_seq) => {
                    write!(f, "no pending segments; waiting_for_seq={waiting_for_seq}")
                }
                None => write!(f, "no pending segments"),
            };
        }

        write!(
            f,
            "waiting_for_seq={:?} first_pending_seq={:?} gap_before_first_pending_bytes={} first_pending_len={} pending_segments={} pending_bytes={} last_pending_seq={:?} last_pending_end_seq={:?} last_pending_len={}",
            self.waiting_for_seq,
            self.first_pending_seq,
            self.first_gap_bytes,
            self.first_pending_len,
            self.pending_segments,
            self.pending_bytes,
            self.last_pending_seq,
            self.last_pending_end_seq,
            self.last_pending_len
        )
    }
}
...
impl TcpHalfStream {
    ...
    pub fn pending_segments_diag(&self) -> TcpPendingSegmentsDiag {
        let waiting_for_seq = self.next_seq.or(self.first_seq);
        let pending_segments = self.segments.len();
        ...
        let Some((&first_pending_seq, first_pending_segment)) = self.segments.first_key_value()
        else {
            return TcpPendingSegmentsDiag {
                waiting_for_seq,
                pending_segments,
                pending_bytes,
                ..Default::default()
        ...
        let (&last_pending_seq, last_pending_segment) = self
            .segments
            .last_key_value()
            .expect("first_key_value returned Some, so last_key_value must also exist");

        let first_gap_bytes = waiting_for_seq
            .filter(|waiting| seq_gt(first_pending_seq, *waiting))
            .map(|waiting| first_pending_seq.wrapping_sub(waiting) as usize)
            .unwrap_or(0);

        TcpPendingSegmentsDiag {
            waiting_for_seq,
            pending_segments,
            pending_bytes,
            first_pending_seq: Some(first_pending_seq),
            first_pending_len: first_pending_segment.data.len(),
            first_gap_bytes,
            last_pending_seq: Some(last_pending_seq),
            last_pending_end_seq: Some(
                last_pending_seq.wrapping_add(last_pending_segment.data.len() as u32),
            ),
            last_pending_len: last_pending_segment.data.len(),
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_pending_segments_diag_reports_head_gap() {
        let mut stream = TcpHalfStream::new();
        ...
        let diag = stream.pending_segments_diag();
        assert_eq!(diag.waiting_for_seq, Some(1005));
        assert_eq!(diag.pending_segments, 1);
        assert_eq!(diag.pending_bytes, 5);
        assert_eq!(diag.first_pending_seq, Some(1011));
        assert_eq!(diag.first_gap_bytes, 6);
        assert_eq!(diag.last_pending_seq, Some(1011));
        assert_eq!(diag.last_pending_end_seq, Some(1016));
    }
