    ...
    if (this.state.hasError) {
      return (
        <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-secondary)' }}>
          <p style={{ fontSize: 15, marginBottom: 16 }}>页面加载出错</p>
          <button
...
function RealtimeBridge({
  ...
  useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('vigilyx:connection-change', { detail: { connected: nowConnected } }))

    if (wasDisconnected && nowConnected) {
      refreshStats?.()
      window.dispatchEvent(new Event('vigilyx:ws-reconnected'))
    }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed.type === 'StatsUpdate') {
        window.dispatchEvent(new CustomEvent('vigilyx:stats-update', { detail: { stats: parsed.data, connected: true } }))
        return
      }
      ...
      if (parsed.type === 'DataSecurityAlert') {
        ...
        window.dispatchEvent(new Event('vigilyx:dashboard-refresh'))
        return
      }
...
function AppContent({ onLogout }: AppContentProps) {
  ...
  const loadStats = useCallback(() => {
    apiFetch('/api/stats')
      ...
      .then(data => {
        if (data.success) {
          window.dispatchEvent(new CustomEvent('vigilyx:stats-update', { detail: { stats: data.data } }))
        }
      })
      .catch(console.error)
  }, [])
  ...
  useEffect(() => {
    ...
    window.addEventListener('vigilyx:stats-cleared', handler)
    return () => window.removeEventListener('vigilyx:stats-cleared', handler)
  }, [loadStats])
  ...
  return (
    <div className="app">
      ...
      <header className="hd">
        <div className="hd-inner">
          ...
          <Link to="/" className="hd-brand">
            ...
            <span className="hd-brand-sub">邮件威胁分析智能化平台</span>
          </Link>

          {/* ── Nav ── */}
          <nav className="hd-nav">
            {/* Core features */}
            <NavLink to="/" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>}>数据总览</NavLink>
            <NavLink to="/emails" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>}>邮件流量</NavLink>
            <NavLink to="/security" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>}>邮件安全</NavLink>
            <NavLink to="/data-security" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>}>数据安全</NavLink>
            <NavLink to="/automation" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg>}>自动处置</NavLink>
            <span className="hd-nav-sep" />
            {/* Supporting features */}
            <NavLink to="/knowledge" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>}>安全知识</NavLink>
            <NavLink to="/community" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>}>开源社区</NavLink>
            <NavLink to="/logs" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>}>审计日志</NavLink>
            <NavLink to="/settings" icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>}>系统设置</NavLink>
          </nav>

          {/* ── Right Controls ── */}
          <div className="hd-actions">
            <Link to="/settings" className="hd-deploy-badge" title="部署模式 — 点击进入设置" onClick={() => window.dispatchEvent(new CustomEvent('vigilyx:navigate-settings', { detail: 'deployment' }))}>
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M2 12h4l3-9 4 18 3-9h6"/></svg>
              <span>{deployMode === 'mta' ? 'MTA 网关' : '镜像监听'}</span>
            </Link>
            <SystemStatusBar />
            <div className={`hd-ws ${connected ? 'hd-ws--on' : 'hd-ws--off'}`} title={connected ? 'WebSocket 已连接' : 'WebSocket 离线'}>
              <span className="hd-ws-dot" />
            </div>
            <ThemeToggle theme={theme} onToggle={toggleTheme} />
            <button className="hd-logout" onClick={onLogout} title="退出登录">
      ...
      <main className="main">
        <ChunkErrorBoundary><Suspense fallback={<div className="page-loading">正在加载页面...</div>}>
          <Routes>
      ...
      <footer className="footer">
        <p>Vigilyx v0.9.0 &mdash; 邮件流量监控系统</p>
      </footer>
    </div>
  )
}
...
function PortalContent({ onLogout }: AppContentProps) {
  ...
  return (
    <div className="app">
      ...
      <header className="hd">
        <div className="hd-inner">
          <Link to="/portal" className="hd-brand">
            <span className="hd-brand-name">Vigilyx</span>
            <span className="hd-brand-sep" />
            <span className="hd-brand-sub">邮件安全管理平台</span>
          </Link>

          <nav className="hd-nav">
            <NavLink to="/portal" activeMatch={p => p.startsWith('/portal') && !p.startsWith('/portal/settings')} icon={<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M21 12c0 1.66-4 3-9 3s-9-1.34-9-3"/><path d="M3 5v14c0 1.66 4 3 9 3s9-1.34 9-3V5"/></svg>}>数据安全</NavLink>
      ...
      <footer className="footer">
        <p>Vigilyx v0.9.0 &mdash; 邮件安全管理平台</p>
      </footer>
    </div>
  )
}
...
function App() {
  ...
  if (!authReady) {
    return <div className="page-loading">正在检查初始化状态...</div>
  }
