  ...
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-blue">{i18n.t('article.tag.basics')}</span>
        <h1>{i18n.t('article.title.mta')}</h1>
        <p className="sk-lead">MTA 是负责在互联网上传递电子邮件的服务器软件。每封邮件从发送到接收，至少经过 2 个 MTA。</p>
      </div>

      <section className="sk-section">
        <h2>{i18n.t('article.section.coreConcepts')}</h2>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.deliveryFlow')}</h2>
        ...
        <div className="sk-flow">
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.sender')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.muaClient')}</div>
          </div>
          ...
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.senderMta')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.exampleSmtpGmail')}</div>
          </div>
          <div className="sk-flow-arrow">
            <span>SMTP :25</span>
          </div>
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.relayMta')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.optionalFilter')}</div>
          </div>
          <div className="sk-flow-arrow">
            <span>SMTP :25</span>
          </div>
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.recipientMta')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.mxLookup')}</div>
          </div>
          <div className="sk-flow-arrow">
            <span>POP3/IMAP</span>
          </div>
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.recipient')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.muaClient')}</div>
          </div>
        </div>
      </section>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.mtaVigilyxRelation')}</h2>
        <p>Vigilyx 支持<strong>旁路镜像</strong>和<strong>MTA 代理</strong>两种接入方式。镜像模式通过抓包观察 MTA 之间的 SMTP 通信；MTA 代理模式则直接作为 SMTP 中继接收邮件，在投递前完成解析与判定。</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.term')}</th><th>{i18n.t('article.table.fullName')}</th><th>{i18n.t('article.table.role')}</th></tr>
            </thead>
            <tbody>
...
export function TopicOpportunisticTLS() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-amber">{i18n.t('article.tag.encryption')}</span>
        <h1>{i18n.t('article.title.opportunisticTls')}</h1>
        <p className="sk-lead">机会性 TLS 是一种"能加密就加密，不能就算了"的策略。它是当今 MTA 之间最主流的加密方式，但存在被降级攻击的风险。</p>
      </div>

      <section className="sk-section">
        <h2>{i18n.t('article.section.howItWorks')}</h2>
        <p>机会性 TLS 的流程如下：</p>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.keyCharacteristics')}</h2>
        <div className="sk-cards-row">
          <div className="sk-info-card">
            <div className="sk-info-card-header good">{i18n.t('article.card.advantages')}</div>
            <ul>
              <li>无需预配置，自动协商</li>
          ...
          <div className="sk-info-card">
            <div className="sk-info-card-header bad">{i18n.t('article.card.risks')}</div>
            <ul>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.downgradeAttack')}</h2>
        ...
        <div className="sk-diagram">
          <div className="sk-diagram-row">
            <span className="sk-diagram-label">{i18n.t('article.diagram.normal')}</span>
            <div className="sk-diagram-flow">
              <code>{i18n.t('article.flow.senderMtaShort')}</code>
              <span className="sk-arrow good">--STARTTLS--&gt;</span>
              <code>{i18n.t('article.flow.recipientMtaShort')}</code>
            </div>
            <span className="sk-badge-inline good">{i18n.t('article.diagram.encrypted')}</span>
          </div>
          <div className="sk-diagram-row">
            <span className="sk-diagram-label">{i18n.t('article.diagram.attack')}</span>
            <div className="sk-diagram-flow">
              <code>{i18n.t('article.flow.senderMtaShort')}</code>
              <span className="sk-arrow bad">--{i18n.t('article.diagram.plaintext')}--&gt;</span>
              <code className="sk-attacker">{i18n.t('article.flow.attacker')}</code>
              <span className="sk-arrow bad">--{i18n.t('article.diagram.plaintext')}--&gt;</span>
              <code>{i18n.t('article.flow.recipientMtaShort')}</code>
            </div>
            <span className="sk-badge-inline bad">{i18n.t('article.diagram.plaintext')}</span>
          </div>
        </div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.vigilyxSniffRelation')}</h2>
        <p>机会性 TLS 对 Vigilyx 的影响：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.scenario')}</th><th>{i18n.t('article.table.vigilyxCanSee')}</th></tr>
            </thead>
            <tbody>
...
export function TopicMandatoryTLS() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-green">{i18n.t('article.tag.encryption')}</span>
        <h1>{i18n.t('article.title.mandatoryTls')}</h1>
        <p className="sk-lead">强制 TLS 要求 MTA 之间的通信<em>必须</em>加密。如果无法建立加密通道，邮件将被拒绝发送，而非降级为明文。</p>
      </div>

      <section className="sk-section">
        <h2>{i18n.t('article.section.whyMandatoryTls')}</h2>
        ...
        <div className="sk-compare">
          <div className="sk-compare-col">
            <div className="sk-compare-header amber">{i18n.t('article.compare.opportunisticTls')}</div>
          ...
          <div className="sk-compare-col">
            <div className="sk-compare-header green">{i18n.t('article.compare.mandatoryTls')}</div>
            <div className="sk-compare-body">
              <p>"必须加密，失败则拒绝发送"</p>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.implementation')}</h2>
        <div className="sk-cards-col">
          <div className="sk-detail-card">
            ...
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.mtaSts')}</h3>
          ...
          <div className="sk-detail-card">
            ...
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.dane')}</h3>
              <p>通过 DNSSEC 在 DNS 中发布 TLSA 记录，绑定邮件服务器的 TLS 证书指纹。发件 MTA 通过 DNS 验证证书真实性。</p>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.dnsTlsa')}</div>
          ...
          <div className="sk-detail-card">
            ...
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.manualTlsPolicy')}</h3>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.fullComparison')}</h2>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.comparison')}</th><th>{i18n.t('article.table.opportunisticTls')}</th><th>{i18n.t('article.table.mandatoryTls')}</th></tr>
            </thead>
            <tbody>
...
export function TopicSTARTTLS() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-purple">{i18n.t('article.tag.protocol')}</span>
        <h1>{i18n.t('article.title.starttls')}</h1>
        <p className="sk-lead">STARTTLS 是一种协议扩展命令，用于将已建立的明文连接就地升级为 TLS 加密连接。它是实现机会性 TLS 的核心机制。</p>
      </div>

      <section className="sk-section">
        <h2>{i18n.t('article.section.starttlsInteraction')}</h2>
        ...
        <div className="sk-smtp-dialog">
          ...
          <div className="sk-smtp-line server highlight"><span className="sk-smtp-dir">S:</span><code>250-STARTTLS</code><span className="sk-smtp-note">-- {i18n.t('article.smtp.serverStarTLS')}</span></div>
          <div className="sk-smtp-line server"><span className="sk-smtp-dir">S:</span><code>250 8BITMIME</code></div>
          <div className="sk-smtp-line client highlight"><span className="sk-smtp-dir">C:</span><code>STARTTLS</code><span className="sk-smtp-note">-- {i18n.t('article.smtp.clientRequestUpgrade')}</span></div>
          <div className="sk-smtp-line server highlight"><span className="sk-smtp-dir">S:</span><code>220 2.0.0 Ready to start TLS</code><span className="sk-smtp-note">-- {i18n.t('article.smtp.beginTlsHandshake')}</span></div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.starttlsVsImplicit')}</h2>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.comparison')}</th><th>{i18n.t('article.table.starttlsExplicit')}</th><th>{i18n.t('article.table.implicitTls')}</th></tr>
            </thead>
            <tbody>
...
export function TopicDSFusion() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-red">{i18n.t('article.tag.riskModel')}</span>
        <h1>{i18n.t('article.title.dsFusion')}</h1>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.whyDsTheory')}</h2>
        <p>传统<strong>贝叶斯方法</strong>用单一概率 P(threat) 表达威胁程度。当一个检测模块"无法判断"时，它只能输出 P=0.5 —— 但这与模块<em>明确认为</em>50%概率是威胁<strong>完全不同</strong>：</p>
        <div className="sk-cards-row">
          <div className="sk-info-card">
            <div className="sk-info-card-header bad">{i18n.t('article.card.problem1Uncertainty')}</div>
            <ul>
          ...
          <div className="sk-info-card">
            <div className="sk-info-card-header bad">{i18n.t('article.card.problem2Conflict')}</div>
            <ul>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.bpaTriple')}</h2>
        <p>在辨识框架 <code>Θ = {'{'}Threat, Normal{'}'}</code> 上定义<strong>基本概率赋值</strong> (Basic Probability Assignment, BPA)，简化为三元组：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.symbol')}</th><th>{i18n.t('article.table.name')}</th><th>{i18n.t('article.table.meaning')}</th><th>{i18n.t('article.table.range')}</th></tr>
            </thead>
            <tbody>
        ...
        <div className="sk-detail-card">
          ...
          <div className="sk-detail-card-content">
            <h3>{i18n.t('article.subtitle.scoreTooBpa')}</h3>
            <p>给定模块输出 <code>(score, confidence)</code>：</p>
            <div className="sk-code-block">
              <div className="sk-code-title">{i18n.t('article.code.bpaConversion')}</div>
        ...
        <div className="sk-detail-card">
          <div className="sk-detail-card-num">{'→'}</div>
          <div className="sk-detail-card-content">
            <h3>{i18n.t('article.subtitle.riskScoreCalc')}</h3>
            <div className="sk-code-block">
              <div className="sk-code-title">{i18n.t('article.code.riskScoreFormula')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.dempsterZadeh')}</h2>
        <p>给定两个 BPA <code>m₁</code> 和 <code>m₂</code>，Dempster 合成规则通过<strong>正交和</strong>计算联合信度：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.dempsterRule')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.murphyFusion')}</h2>
        <p>Murphy 修正通过<strong>证据距离加权平均</strong>预处理解决 Zadeh 悖论：</p>
        <div className="sk-cards-col">
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">1</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.jousselmeDist')}</h3>
              <p>度量两个 BPA 之间的差异程度：</p>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.jousselmeDist')}</div>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">2</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.simCredWeight')}</h3>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.credCalc')}</div>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">3</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.weightedAvgSelfCombine')}</h3>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.murphyFlow')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.copulaDependency')}</h2>
        <p>Dempster 合成规则假设证据源之间<strong>相互独立</strong>。但实际系统中引擎间存在特征共享：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.enginePair')}</th><th>{i18n.t('article.table.sharedFeature')}</th><th>{i18n.t('article.table.correlationCoeff')}</th></tr>
            </thead>
            <tbody>
        ...
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.copulaDiscount')}</div>
          <pre>对相关系数 ρ_ij 较高的引擎对：{'\n'}{'\n'}b_new = b × (1 - ρ)     // 信度打折{'\n'}u_new = u + (b + d) × ρ // 不确定度增加{'\n'}{'\n'}直觉: 相关的引擎给出的证据要打折扣，{'\n'}      因为它们不是真正独立的信息源</pre>
        </div>
      </section>

      <section className="sk-section">
        <h2>{i18n.t('article.section.eightEngineArch')}</h2>
        <p>系统采用八个互补引擎覆盖邮件威胁的不同维度，确保无单一盲区：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.engine')}</th><th>{i18n.t('article.table.coreName')}</th><th>{i18n.t('article.table.coreCapability')}</th><th>{i18n.t('article.table.coveredThreat')}</th></tr>
            </thead>
            <tbody>
        ...
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.finalRiskScore')}</div>
          <pre>Risk_single = b_final + η × u_final{'\n'}{'\n'}其中 b_final, u_final 来自八引擎 Murphy 融合后的 BPA{'\n'}η = 0.7 (风险态度参数)</pre>
        </div>
      </section>

      <section className="sk-section">
        <h2>{i18n.t('article.section.adversarialRobustness')}</h2>
        <p>攻击者可能针对性地规避某个引擎。<strong>多样性约束</strong>确保系统不过度依赖任何单一引擎：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.diversityConstraint')}</div>
...
export function TopicTemporalEVT() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-red">{i18n.t('article.tag.riskModel')}</span>
        <h1>{i18n.t('article.title.temporalEvt')}</h1>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.temporalPosition')}</h2>
        <p>旁路监测系统的独特优势在于可以观察到<strong>所有邮件流量的完整时间序列</strong>。</p>
        <div className="sk-compare">
          <div className="sk-compare-col">
            <div className="sk-compare-header amber">{i18n.t('article.compare.singleEmailAnalysis')}</div>
            <div className="sk-compare-body">
              <p>"这封邮件是否危险？"</p>
          ...
          <div className="sk-compare-vs">VS</div>
          <div className="sk-compare-col">
            <div className="sk-compare-header green">{i18n.t('article.compare.temporalLayer')}</div>
            <div className="sk-compare-body">
              <p>"这个发件人的行为是否在逐渐变化？"</p>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.cusumDetection')}</h2>
        <p><strong>CUSUM</strong> (Cumulative Sum) 是经典的序列变点检测算法，用于检测发件人风险水平的<em>突然偏移</em>：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.cusumFormula')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.dualEwma')}</h2>
        <p><strong>指数加权移动平均</strong> (EWMA) 用两个不同速度追踪发件人的行为基线：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.dualEwma')}</div>
        ...
        <div className="sk-diagram">
          <div className="sk-diagram-row">
            ...
            <span className="sk-badge-inline good">{i18n.t('article.diagram.stable')}</span>
          </div>
          <div className="sk-diagram-row">
            <span className="sk-diagram-label">{i18n.t('article.diagram.gradual')}</span>
            <div className="sk-diagram-flow">
              <code>E_fast {'>'} E_slow</code>
              <span className="sk-arrow bad">→ drift ↑</span>
            </div>
            <span className="sk-badge-inline bad">{i18n.t('article.diagram.drift')}</span>
          </div>
        </div>
        <div className="sk-callout warning">
          <span className="sk-callout-icon">
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.entityRiskAccum')}</h2>
        <p>为每个发件人和域名维护一个<strong>持续衰减的风险累积值</strong>：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.entityRisk')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.hmmFiveState')}</h2>
        <p><strong>隐马尔可夫模型</strong> (Hidden Markov Model) 用于推断发件人-收件人对之间的攻击阶段演进：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.state')}</th><th>{i18n.t('article.table.phase')}</th><th>{i18n.t('article.table.feature')}</th><th>{i18n.t('article.table.riskWeight')}</th></tr>
            </thead>
            <tbody>
        ...
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.forwardAlgo')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.commGraphAnomaly')}</h2>
        <p>维护一个<strong>有向加权图</strong>：节点=邮箱地址，边=通信记录（含频率和风险历史）。检测三种异常模式：</p>
        <div className="sk-cards-col">
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">1</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.massPhishing')}</h3>
              <p>一个首次出现的发件人短时间内向大量收件人发送邮件。</p>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.detectionCondition')}</div>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">2</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.becLateral')}</h3>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">3</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.exfilBurst')}</h3>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.gpdEvt')}</h2>
        <p><strong>广义帕累托分布</strong> (Generalized Pareto Distribution) 用于建模风险分数的极端尾部行为：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.gpdDist')}</div>
          <pre>P(X {'>'} x | X {'>'} u) = (1 + ξ(x-u)/σ)^{'{-1/ξ}'}{'\n'}{'\n'}u = 历史风险分数的 95 分位数 (阈值){'\n'}ξ = 形状参数 (ξ{'>'}0 为重尾 Fréchet 分布){'\n'}σ = 尺度参数 (尾部展宽程度){'\n'}{'\n'}参数通过 PWM (概率加权矩) 方法拟合</pre>
        </div>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.returnPeriodCvar')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.p0p3AlertGrading')}</h2>
        <p>基于<strong>期望损失</strong>和多维信号的四级告警体系：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.expectedLoss')}</div>
          <pre>EL = Risk_final × Impact_target{'\n'}{'\n'}Impact 权重 (按收件人角色):{'\n'}  高管: 5.0 | 财务: 4.5 | IT管理: 4.0{'\n'}  客户经理: 3.5 | 一般员工: 1.0</pre>
        </div>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.level')}</th><th>{i18n.t('article.table.triggerCondition')}</th><th>{i18n.t('article.table.responseRequirement')}</th></tr>
            </thead>
            <tbody>
              <tr>
...
export function TopicModulePipeline() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-red">{i18n.t('article.tag.riskModel')}</span>
        <h1>{i18n.t('article.title.modulePipeline')}</h1>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.moduleArchitecture')}</h2>
        <p>每个安全模块实现 <code>SecurityModule</code> trait，包含三个核心方法：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.method')}</th><th>{i18n.t('article.table.function')}</th><th>{i18n.t('article.table.returnValue')}</th></tr>
            </thead>
            <tbody>
        ...
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.moduleResult')}</div>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.fourteenModules')}</h2>
        <p>模块按八引擎架构分组，覆盖邮件威胁的不同维度：</p>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.engine')}</th><th>{i18n.t('article.table.moduleId')}</th><th>{i18n.t('article.table.coreName')}</th><th>{i18n.t('article.table.detectionCapability')}</th></tr>
            </thead>
            <tbody>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.dagOrchestrator')}</h2>
        ...
        <div className="sk-flow">
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.emailInput')}</div>
          ...
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.layer0')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.layer0Sub')}</div>
          </div>
          <div className="sk-flow-arrow">
            <span>results</span>
          </div>
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.dsFusion')}</div>
            <div className="sk-flow-sub">{i18n.t('article.flow.dsFusionSub')}</div>
          </div>
          <div className="sk-flow-arrow">
            <span>verdict</span>
          </div>
          <div className="sk-flow-node">
            ...
            <div className="sk-flow-label">{i18n.t('article.flow.verdictOutput')}</div>
        ...
        <div className="sk-cards-row">
          <div className="sk-info-card">
            <div className="sk-info-card-header good">{i18n.t('article.card.parallelExecution')}</div>
            <ul>
          ...
          <div className="sk-info-card">
            <div className="sk-info-card-header bad">{i18n.t('article.card.faultTolerance')}</div>
            <ul>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.scoreToTriple')}</h2>
        ...
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.scenario')}</th><th>{i18n.t('article.table.module')}</th><th>{i18n.t('article.table.score')}</th><th>{i18n.t('article.table.confidence')}</th><th>b</th><th>d</th><th>u</th><th>{i18n.t('article.table.interpretation')}</th></tr>
            </thead>
            <tbody>
              <tr>
                <td>高危确认</td>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.engineMapping')}</h2>
        <p>14 个模块按功能映射到 8 个概念引擎。对于<strong>多模块引擎</strong>（如 B 有 5 个模块、D 有 3 个），需要先在引擎内部通过 Dempster 合成规则逐步合成：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.engineBComposition')}</div>
        ...
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.engine')}</th><th>{i18n.t('article.table.containedModules')}</th><th>{i18n.t('article.table.moduleCount')}</th><th>{i18n.t('article.table.compositionMethod')}</th></tr>
            </thead>
            <tbody>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.fullFusionPipeline')}</h2>
        <p>引擎内合成完成后，进入<strong>跨引擎 Murphy 融合管线</strong>，共 6 步：</p>
        <div className="sk-cards-col">
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">1</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.copulaDiscount')}</h3>
              <p>对每个引擎，找到与之最相关引擎的相关系数 <code>max_ρ</code>。如果 <code>max_ρ {'>'} 0.1</code>，对其 BPA 进行折扣：</p>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.discountFormula')}</div>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">3</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.credWeight')}</h3>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.murphyWeight')}</div>
          ...
          <div className="sk-detail-card">
            ...
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.diversityConstraint')}</h3>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.weightClipping')}</div>
                <pre>约束: w_i ≤ 0.4 × Σw_j{'\n'}超出部分按比例重分配给未触顶的引擎{'\n'}重新归一化使 Σw_i = 1.0</pre>
              </div>
            </div>
          </div>
          <div className="sk-detail-card">
            ...
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.weightedAvg')}</h3>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.murphyWeightedAvg')}</div>
          ...
          <div className="sk-detail-card">
            ...
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.selfCombine')}</h3>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.dempsterSelfCombine')}</div>
      ...
      <section className="sk-section">
        ...
        <div className="sk-diagram">
          ...
          <div className="sk-diagram-row">
            <span className="sk-diagram-label">{i18n.t('article.diagram.afterDiscount')}</span>
            <div className="sk-diagram-flow">
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.finalVerdict')}</h2>
        <p>经过 D-S Murphy 融合和信任折扣后，最终风险分数映射到五级威胁等级：</p>
        <div className="sk-code-block">
          <div className="sk-code-title">{i18n.t('article.code.finalRiskCalc')}</div>
          <pre>Risk_single = b_final + η × u_final  (η = 0.7)</pre>
        </div>
        <div className="sk-table-wrap">
          <table className="sk-table">
            <thead>
              <tr><th>{i18n.t('article.table.level')}</th><th>{i18n.t('article.table.threshold')}</th><th>{i18n.t('article.table.meaning')}</th></tr>
            </thead>
            <tbody>
...
export function TopicSPF() {
  return (
    <article className="sk-article">
      <div className="sk-article-header">
        <span className="sk-tag sk-tag-cyan">{i18n.t('article.tag.authentication')}</span>
        <h1>{i18n.t('article.title.spfDkimDmarc')}</h1>
        <p className="sk-lead">这三种机制共同解决一个问题：<strong>这封邮件真的是它声称的发件人发送的吗？</strong></p>
      </div>

      <section className="sk-section">
        <h2>{i18n.t('article.section.threeRelations')}</h2>
        <div className="sk-cards-col">
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">1</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.spf')}</h3>
              <p>在 DNS 中声明"哪些 IP 有权代表我的域名发送邮件"。收件 MTA 检查发件 IP 是否在授权列表中。</p>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.dnsTxtRecord')}</div>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">2</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.dkim')}</h3>
              <p>发件 MTA 使用私钥对邮件头和正文签名，收件方通过 DNS 获取公钥验证。可确保邮件在传输中未被篡改。</p>
              <div className="sk-code-block">
                <div className="sk-code-title">{i18n.t('article.code.dkimSignature')}</div>
          ...
          <div className="sk-detail-card">
            <div className="sk-detail-card-num">3</div>
            <div className="sk-detail-card-content">
              <h3>{i18n.t('article.subtitle.dmarc')}</h3>
      ...
      <section className="sk-section">
        <h2>{i18n.t('article.section.tlsDifference')}</h2>
        <div className="sk-callout warning">
          <span className="sk-callout-icon">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
          </span>
...
export function TopicPhishingDetection() {
  return (
    <article className="sk-article">
      <section id="多维检测架构"><h2>{i18n.t('article.section.multiDimDetection')}</h2>
      ...
      <section id="内容分析引擎"><h2>{i18n.t('article.section.engineBContent')}</h2>
      ...
      <section id="URL链接分析引擎"><h2>{i18n.t('article.section.engineDUrl')}</h2>
      ...
      <section id="语义与行为分析"><h2>{i18n.t('article.section.engineFGH')}</h2>
      ...
      <section id="安全熔断器"><h2>{i18n.t('article.section.circuitBreaker')}</h2>
...
export function TopicIocIntel() {
  return (
    <article className="sk-article">
      <section id="IOC类型与来源"><h2>{i18n.t('article.section.iocTypesAndSources')}</h2>
      ...
      <section id="自动记录与防护"><h2>{i18n.t('article.section.autoRecordProtection')}</h2>
      ...
      <section id="外部情报源"><h2>{i18n.t('article.section.externalIntelSources')}</h2>
      ...
      <section id="白名单管理"><h2>{i18n.t('article.section.whitelistManagement')}</h2>
...
export function TopicAiNlp() {
  return (
    <article className="sk-article">
      <section id="双模型优先级"><h2>{i18n.t('article.section.dualModelPriority')}</h2>
      ...
      <section id="零样本分类"><h2>{i18n.t('article.section.zeroShotClassification')}</h2>
        <p>将邮件内容与 5 个候选标签组成前提-假设对，通过 NLI 模型判断蕴含关系。中文标签：钓鱼邮件/诈骗邮件/商务欺诈/垃圾邮件/正常邮件。恶意概率 = P(phishing) + P(scam) + P(bec)。CJK 占比 &gt; 30% 用中文标签集。</p>
      </section>
      <section id="LoRA微调"><h2>{i18n.t('article.section.loraFinetune')}</h2>
      ...
      <section id="NLP与规则协作"><h2>{i18n.t('article.section.nlpRuleCooperation')}</h2>
...
export function TopicSoarAlerts() {
  return (
    <article className="sk-article">
      <section id="P0-P3四级告警"><h2>{i18n.t('article.section.p0p3FourLevelAlert')}</h2>
        <table className="sk-table">
          <thead><tr><th>{i18n.t('article.table.level')}</th><th>{i18n.t('article.table.condition')}</th><th>{i18n.t('article.table.meaning')}</th></tr></thead>
          <tbody>
      ...
      <section id="期望损失与极值理论"><h2>{i18n.t('article.section.expectedLossEvt')}</h2>
        <p>EVT 使用 GPD 对风险尾部建模：收集最近 2000 样本的 95 分位数以上超出量，估计 GPD 参数，计算 VaR、CVaR 和回归期 T。T 越大，事件越罕见、越值得关注。</p>
      </section>
      <section id="SOAR处置规则"><h2>{i18n.t('article.section.soarDispositionRules')}</h2>
...
export function TopicDataSecurity() {
  return (
    <article className="sk-article">
      <section id="HTTP流量捕获"><h2>{i18n.t('article.section.httpTrafficCapture')}</h2>
      ...
      <section id="三类泄露检测"><h2>{i18n.t('article.section.threeLeakDetection')}</h2>
        <ul>
      ...
      <section id="DLP扫描"><h2>{i18n.t('article.section.dlpScan')}</h2>
